    ...
    def forward(self, x, y, z, D, domain):
        ...
        for i in range(self.stage2_num_branches):
            if self.transition1[i] is not None:
                x_list.append(self.transition1[i](x))##? 48 64 64；? 96 32 32
                ...
                x_list_msi.append(y)
                x_list_sar.append(z)
        y_list = self.stage2(x_list)#原尺度和二分之一尺度的多尺度特征
        ...
        for i in range(self.stage3_num_branches):
            if self.transition2[i] is not None:
                ...
                x_list.append(y_list[i])##? 48 64 64；? 96 32 32；? 192 16 16
                x_list_msi.append(y_list_msi[i])
                x_list_sar.append(y_list_sar[i])
        y_list = self.stage3(x_list)#原尺度、二分之一、四分之一尺度的多尺度特征
        ...
        x_list_msi = []
        x_list_sar = []## 不同分辨率的融合
        # 阶段2、3和4是HR-Net architecture，水平和垂直方向表示网络的深度和特征图尺度（阶段1改为三个图像通道一致处理）
        # 每个阶段都比上一个阶段新增一条直流，新增的支流比上层支流分辨率减半，但通道数增加一倍。
        for i in range(self.stage4_num_branches):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])##? 48 64 64；? 96 32 32；? 192 16 16；? 384 8 8
                x_list_msi.append(y_list_msi[i])
                x_list_sar.append(y_list_sar[i])
        x = self.stage4(x_list)##原尺度、二分之一、四分之一、八分之一尺度的多尺度特征
        ...
        x = torch.cat([x[0], x1, x2, x3, y[0], y1, y2, y3, z[0], z1, z2, z3], 1)## 4 2160 64 64
