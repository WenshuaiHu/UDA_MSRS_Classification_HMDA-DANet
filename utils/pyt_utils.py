...
def recover(y_true, y_pred, dataset, patch_size, save_path):
    y_true = np.array(y_true).flatten()
    y_pred = np.array(y_pred).flatten()
    mask = (y_true > 0) & (y_true <= 13)
    ...
    y_pred = patch_concat(y_pred, h, w, patch_size)# 2456 811
    #print(1111111111, y_pred.shape, y_pred)#
    scio.savemat(save_path + '/y_pred_result.mat', {'output': y_pred})
    scio.savemat(save_path + '/y_true_result.mat', {'output': y_true})
